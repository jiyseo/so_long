...
/*   Updated: 2022/07/03 05:11:06 by jiyseo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	map_paint(t_game *game)
{
	...
	walk_str = ft_itoa(game->walk_cnt);
	mlx_string_put(game->mlx, game->win, 10 - 1, 20, 0, walk_str);
	mlx_string_put(game->mlx, game->win, 10 + 1, 20, 0, walk_str);
	mlx_string_put(game->mlx, game->win, 10, 20 - 1, 0, walk_str);
	mlx_string_put(game->mlx, game->win, 10, 20 + 1, 0, walk_str);
	mlx_string_put(game->mlx, game->win, 10, 20, 0xFF0000, walk_str);
	free(walk_str);
}
